...
class IntCodeComputer:
    ...
    def __init__(self, memory = 10000):
        ...
        self.memory = memory
        self.intcodes = [' ' for x in range(int(self.memory))]        
    ...
    def run_program(self, inputs = [], display = True):
        ...
        while int(intcode) != 99 and self.instruction_pointer < len(self.intcodes):
            ...
            ret = self.process_intcode(intcode, inputs, display)

            if ret < 0: break

        return self.outputs
        
    def process_intcode(self, intcode, inputs = [], display = True):
        ...
        if opcode == '04':
            ...
            if display: print('Output: ', operand1)
